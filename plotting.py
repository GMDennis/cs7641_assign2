...
def plot_data(title, data, column_prefixes=None, validate_only=False, nn_curve=False, clear_existing=True,
    ...
    if column_prefixes is not None:
            ...
            if not nn_curve:
                ...
                if not validate_only:
                    plt.plot(data.index, train, '-', linewidth=1, markersize=1,
                             label='Train {} {}'.format(column_prefix, y_label), alpha=0.7)
                    plt.plot(data.index, val, '-', linewidth=1, markersize=1,
                             label='CV {} {}'.format(column_prefix, y_label), alpha=0.7)
                else:
                    plt.plot(data.index, test, '-', linewidth=1, markersize=1,
                             label='{} {}'.format(column_prefix, y_label), alpha=0.7)
        ...
        if not nn_curve:
            ...
            plt.fill_between(data.index, mean - std,
                             mean + std, alpha=0.2)
            plt.plot(data.index, mean, '-', linewidth=1, markersize=1, label=None, alpha=0.7)
        else:
            train = data['f1_trg']
            val = data['f1_val']
            plt.plot(data.index, train, '-', linewidth=1, markersize=1,
                     label='Train {}'.format(y_label), alpha=0.7)
            plt.plot(data.index, val, '-', linewidth=1, markersize=1,
                     label='CV {}'.format(y_label), alpha=0.7)
...
def plot_best_curves(problem_name, files, output_dir, nn_curve=False):
    ...
    for algo in files:
        ...
        file = files[algo][0]
#        if algo == 'RHC':
#            file='./output/NN_OUTPUT/NN_RHC_LOG.csv'
#        else:
#            file = files[algo][0]
        print(file)
    ...
    for algo in files:
        #if algo == 'RHC':
        #    files[algo] = ['./output/NN_OUTPUT/NN_RHC_LOG.csv']
